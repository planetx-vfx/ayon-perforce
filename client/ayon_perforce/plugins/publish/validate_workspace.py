...
class ValidateWorkspace(pyblish.api.ContextPlugin):
    ...
    def process(self, context):
        ...
        workspace_dir = PerforceRestStub.get_workspace_dir(workspace_name=ws_name)
        ...
        # validate stream
        stream = PerforceRestStub.get_stream(workspace_name=ws_name)
        if not stream:
            msg = (
...
class UncommittedChangesRepairer(ErrorMessageBox):
    ...
    def __init__(
        self, uncommitted_changes: list, workspace_dir: str, workspace_name: str
    ):
