...
class P4Workspace:
    ...
    def switch(self, force: bool = False) -> None:
        ...
        if self.depot not in P4.available_depots():
            ...
            msg = f"Depot `{self.depot}` does not exist on the Perforce server."
        ...
        if force:
            ...
            if P4Workspace.opened_files() and curr_ws.stream != self.stream:
                raise RuntimeError("Workspace has opened files. Can't switch streams.")
...
def get_local_login() -> tuple[None, None] | tuple[Any, Any]:
